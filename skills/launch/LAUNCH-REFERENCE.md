...
```
prompt → vibes app → Clerk setup → sell transform → Cloudflare deploy (auto-provisions Connect) → browser test
```
...
- `.env` has Clerk keys → skip T2 (Connect auto-deploys with app)
