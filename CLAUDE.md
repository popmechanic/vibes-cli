...
components/             → build-components.js → cache/vibes-menu.js
                                                       ↓
templates/base/template.html  ←── shared code (components, CSS, imports)
         +
...
| `templates/base/template.html` | Base template with shared code. Edit for all skills. |
...
The import map in `templates/base/template.html` is the authoritative source for current package versions (`esm.sh/stable/` URLs, `@necrodome/fireproof-clerk@0.0.3`, React 19.2.4).
...
The authoritative import map is defined in `templates/base/template.html` — read it there, not here. Key rule: `?external=react,react-dom` is REQUIRED on any esm.sh package that depends on React. After editing the base template, run `node scripts/merge-templates.js --force` to regenerate.
...
| `templates/base/template.html` | Base template with shared code (components, CSS, imports) |
...
The plugin uses JSX with Babel runtime transpilation (matching vibes.diy). See `templates/base/template.html` for the `<script type="text/babel">` pattern.
...
See `templates/base/template.html` for the current authoritative import map. The key points:
...
1. Update import map in `templates/base/template.html` to use esm.sh URLs:
   ```json
