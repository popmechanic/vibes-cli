...
```bash
LATEST=`ls ~/.claude/plugins/cache/vibes-diy/vibes/ | sort -V | tail -1` && cd ~/.claude/plugins/cache/vibes-diy/vibes/$LATEST/scripts && [ -d node_modules/esbuild ] || npm install && cd .. && node scripts/sync.js --force
```
