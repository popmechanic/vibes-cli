...
async function phase3ServerSetup(args) {
  ...
  try {
    ...
    // Add no-cache headers for index.html so deploys are visible immediately
    const cacheConf = `# Vibes cache control - auto-generated by deploy-exe.js
location = /index.html {
...
location = / {
    ...
    await runCommand(client, `cat <<'CACHEEOF' | sudo tee /etc/nginx/vibes-cache.conf\n${cacheConf}\nCACHEEOF`);
    const cacheIncludeCheck = await runCommand(client, 'grep -q "include /etc/nginx/vibes-cache.conf" /etc/nginx/sites-available/default && echo "EXISTS" || echo "NOT_FOUND"');
    if (cacheIncludeCheck.stdout.includes('NOT_FOUND')) {
      await runCommand(client, `sudo sed -i '/^[[:space:]]*server[[:space:]]*{/a\\    include /etc/nginx/vibes-cache.conf;' /etc/nginx/sites-available/default`);
    }
    const cacheNginxTest = await runCommand(client, 'sudo nginx -t 2>&1');
    if (cacheNginxTest.code === 0) {
