...
2. **Detects project state** — checks `$PWD` for:
   - `.env` with OIDC credentials and Connect URLs
...
skills/vibes/template.delta.html  ← Skill-specific code (OIDC auth wrapper)
                    ↓
...
- `VITE_OIDC_AUTHORITY` (must be an HTTPS URL)
...
|---------------------|---------------|
| `__VITE_OIDC_AUTHORITY__` | `VITE_OIDC_AUTHORITY` from .env |
...
- Skill-specific code (OIDC auth wrapper, sharing bridge)
...
### Step 6.4: OIDC Authentication

The template's delta code wraps the app in `OIDCProvider`:

1. `OIDCProvider` initializes with the authority URL and client ID from `window.__VIBES_CONFIG__`
...
User prompt ──→ LLM reads:                    .env (OIDC credentials,
